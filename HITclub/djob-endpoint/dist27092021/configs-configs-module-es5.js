...
var ConfigsFormComponent = /** @class */ (function () {
    function ConfigsFormComponent(configsService, notif) {
    ...
    ConfigsFormComponent.prototype.ngOnInit = function () {
        ...
        this.configsService.getConfigs()
            ...
            for (var i = 0; i < confData.length; i++) {
                _this.configs[confData[i].param] = confData[i].value;
                if (confData[i].param === 'return_to_the_application_customer_texts') {
                    var tmparr = confData[i].value ? confData[i].value.split('|') : [];
                    _this.customer_push_texts = [];
                    for (var j = 0; j < tmparr.length; j++) {
                        _this.customer_push_texts.push({ val: tmparr[j] });
                    }
                }
                if (confData[i].param === 'return_to_the_application_executor_texts') {
                    var tmparr = confData[i].value ? confData[i].value.split('|') : [];
                    _this.executor_push_texts = [];
                    for (var j = 0; j < tmparr.length; j++) {
                        _this.executor_push_texts.push({ val: tmparr[j] });
                    }
                }
            }
        });
    };
    ConfigsFormComponent.prototype.saveConfigs = function () {
        var _this = this;
        this.configs['return_to_the_application_executor_texts'] = '';
        if (this.executor_push_texts.length) {
            for (var i = 0; i < this.executor_push_texts.length; i++) {
        ...
        if (this.customer_push_texts.length) {
            for (var i = 0; i < this.customer_push_texts.length; i++) {
    ...
    ConfigsFormComponent.prototype.addCustomerPushMessage = function () {
        this.customer_push_texts.push({ val: '' });
    };
    ConfigsFormComponent.prototype.deleteCustomerPushMessage = function (text) {
        this.customer_push_texts = this.customer_push_texts.filter(function (_text) { return text !== _text; });
    };
    ConfigsFormComponent.prototype.addExecutorPushMessage = function () {
        this.executor_push_texts.push({ val: '' });
    };
    ConfigsFormComponent.prototype.deleteExecutorPushMessage = function (text) {
        this.executor_push_texts = this.executor_push_texts.filter(function (_text) { return text !== _text; });
    };
    ConfigsFormComponent.ctorParameters = function () { return [
...
var ConfigsRoutingModule = /** @class */ (function () {
    function ConfigsRoutingModule() {
    }
