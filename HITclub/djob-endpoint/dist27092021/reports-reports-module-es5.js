...
var ReportsListComponent = /** @class */ (function () {
    function ReportsListComponent(reportsService, modalService) {
    ...
    ReportsListComponent.prototype.refreshReports = function (reinit) {
        var _this = this;
        ...
        this.reportsService.getlist(options)
            .then(function (list) {
            if (reinit) {
                _this.countReports = 0;
                _this.reportsService.getCount(options)
                    .then(function (countData) {
                    _this.countReports = countData;
                });
            }
            _this.list = list;
        });
    };
    ReportsListComponent.prototype.isEmpty = function (arg) {
        for (var item in arg) {
            return false;
        }
        return true;
    };
    ReportsListComponent.prototype.pageChanged = function (event) {
        this.range.start = (event - 1) * this.countShowInPage;
        this.refreshReports(false);
    };
    ReportsListComponent.prototype.changeFilter = function (key) {
        this.filters[key] = !this.filters[key];
        this.refreshReports();
    };
    ReportsListComponent.prototype.getTextInList = function (text) {
        var ret_len = 80;
        return text.length > ret_len ? text.substr(0, 80) + '...' : text;
    };
    ReportsListComponent.prototype.openReport = function (content, report) {
        this.selectedReport = report;
        this.modalService.open(content);
    };
    ReportsListComponent.prototype.saveReportStatus = function () {
        var _this = this;
        this.reportsService.updateStatus(this.selectedReport.id, { status: this.selectedReport.status })
            .then(function (upData) {
            _this.modalService.dismissAll();
            _this.refreshReports(false);
        });
    };
